...
prev_translation_button = widgets.Button(
...
## Add functions for quick annotations of similar versions of a verse

# Translation
def use_prev_translation(b):
    matching_index = find_matching_row(df, current_index)
    if matching_index is not None:
        prev_translation = df.at[matching_index, 'translation']
        annotation_input.value = prev_translation
        df.at[current_index, 'translation'] = prev_translation
        # Optionally, automatically move to the next column or row here

prev_translation_button.on_click(use_prev_translation)



# ### Step 4.2 Functions to Display Data and Widgets with Navigation and Iteration Logic

def find_matching_row(df, current_index):
    current_row = df.iloc[current_index]
    matching_criteria = ['lex', 'book', 'chapter', 'verse', 'stem', 'tense']

    for index, row in df.iterrows():
        if index != current_index and row['scroll'] != current_row['scroll']:
            if all(row[criteria] == current_row[criteria] for criteria in matching_criteria):
                return index
    return None
...
def display_row(row_index, col_index):
    ...
    if columns_to_annotate[col_index] == 'translation':
        matching_index = find_matching_row(df, current_index)
    
    if matching_index is not None:
        display(prev_translation_button)
