...
class AnnotationTool:
    ...
    def auto_fill_in(self, row_index, col_index):
        """"""
        ...
        if row.complement in self.auto_cache:
            values = self.auto_cache[row.complement]
            for column, value in zip(AUTO_COLUMNS, values):
                self.df.iat[row_index, self.df.columns.get_loc(column)] = value
                

    def display_row(self, row_index, col_index):
        
        self.auto_fill_in(row_index, col_index)
...
##################################################
    ...
    def navigate_row(self, offset):
        ...
        values = [row[column] for column in AUTO_COLUMNS]
        
        if "" not in values and None not in values:
            self.auto_cache[row.complement] = values
            with open(AUTO_CACHE_PATH, "w") as auto_cache_file:
                json.dump(self.auto_cache, auto_cache_file)
    ...
    def _use_prev(self, b, column_name):
        matching_index = self.find_matching_row()
        if matching_index is not None:
            prev_content = self.df.at[matching_index, column_name]
            self.annotation_input.value = prev_content
            self.df.at[self.current_index, column_name] = prev_content
    ...
    def use_prev_translation(self, b):
        return self._use_prev("translation")
         
        
    def use_prev_cmpl_lex(self, b):
        return self._use_prev("cmpl_lex")

    def use_prev_comment(self, b):
        return self._use_prev(b, "comments")
