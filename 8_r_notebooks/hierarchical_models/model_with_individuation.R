...
pri_manual <- c(
  ...
  # cmpl_indiv (multiple levels; ref = baseline)
...
### Model 1 - individuation

# model code

motion_verb_individuation <- brm(
  cmpl_constr ~ cmpl_anim + cmpl_det + cmpl_complex + cmpl_indiv + motion_type +
    verse_genre + era_style +
...
saveRDS(motion_verb_individuation, file = "models/motion_verb_individuation.rds")
...
### Model 2 - Best model + individuation: cmpl_det + cmpl_anim + cmpl_complex + verse_genre + era_style + cmpl_indiv

### # Get priors for book_scroll and lexeme as nested factors

gp_2 <- get_prior(
  cmpl_constr ~ cmpl_det + cmpl_anim + cmpl_complex + verse_genre + era_style + cmpl_indiv +
    (1 | book_scroll) + (1 | lex), # crossed factors
  data = df0,
  family = categorical(link = "logit", refcat = "prep")
)
...
priors_2 <- c(pri_manual_2, pri_re) # priors are different from the first model because motion_type is removed
priors_2
...
# model code
motion_verb_best_individuation <- brm(
  cmpl_constr ~ cmpl_det + cmpl_anim + cmpl_complex + verse_genre + era_style + cmpl_indiv +
    (1 | book_scroll) + (1 | lex), # crossed factors
  data = df0,
  family = categorical(link = "logit", refcat = "prep"),
...
summary(motion_verb_best_individuation)

# Notes adapt_delta = 0.8 and max_treedepth = 10 ==> 15 divergent transitions

# save the fitted model to a file
saveRDS(motion_verb_best_individuation, file = "models/motion_verb_best_individuation.rds")
